...
do
j=1
if [[ $fmode == zh_e2e2 ]]; then
	Work=/besfs/groups/higgs/users/cuizw/results
else 
	echo "err"
fi
echo $Work
...
cd $path
	...
	do
		...
		cp -rf $SimuWork/skim_stdsig_lcio.xml $SimuWork/P"$fmode"_$j.xml > /dev/null
		h=0
		k=0
		while [ "$h" -le "$k" ]
		do
			...
			temp=`printf "$x"`
			InputFile=$Work/"$fmode".e0.p0.000"$temp".stdhep
			if [ -f $InputFile ]
			then
