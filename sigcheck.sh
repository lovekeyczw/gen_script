...
do
	...
	do
		...
		echo $num
		cd $stdhepfile
		gennum=`anajob P"$fmode".e0.p0_"$num".slcio | tail -2 | head -1 | awk -F ' ' '{print($1)}'`
		cd $recfolder
		if [ -f P"$fmode".e0.p0_"$num"_rec.slcio ]
		then
			numrec=1
			TorF=`lcio_event_counter P"$fmode".e0.p0_"$num"_rec.slcio | tail -1 | wc -w`
			if [ "$TorF" -eq "1" ]
			then
				recnum=`lcio_event_counter P"$fmode".e0.p0_"$num"_rec.slcio | tail -1`
			else
				recnum=-1
			fi
		else
		...
		then
			...
			cd $simfolder
			rm -f P"$fmode".e0.p0_"$num"_sim.slcio > /dev/null && echo "$_ not Exist Or Removed"
			cd $recfolder
			rm -f P"$fmode".e0.p0_"$num"_rec.slcio > /dev/null && echo "$_ not Exist Or Removed"
			...
			file=$stdhepfile/P"$fmode".e0.p0_"$num".slcio
			file2=/$simfolder/P"$fmode".e0.p0_"$num"_sim.slcio
			file3=/$recfolder/P"$fmode".e0.p0_"$num"_rec.slcio
